...
class MAPPO_Buffer:
    ...
    def generate_batches(self):
        ...
        return (np.array(self.obs),
                ...
                np.array(self.dones),
                np.array(self.states).astype(np.float32),
                batches)

    def store_memory(self, obs, action, probs, vals, reward, done, state):
    ...
    def clear_memory(self):
        ...
        self.states = np.zeros((self.buffer_size, np.prod(self.state_shape)), dtype=np.bool_)
        self.idx = 0
