...
class MFQ_Agent(Base_Agent):
    def __init__(
        ...
        obs_dim = np.array(self.env.observation_spaces[self.agent_name].shape).prod()
        network_layers = self._get_network_layers(input_dim + obs_dim, hidden_dims, output_dim)
    ...
    def forward(self, observations: torch.Tensor, actions_mean: torch.Tensor):
        ...
        return self.network(torch.cat((observations, actions_mean), dim=-1))
    ...
    def target(self, observations: torch.Tensor, actions_mean: torch.Tensor):
        ...
        # return self.cat_layer(torch.cat((agent_values, mean_values), dim=-1))
        return self.target_network(torch.cat((observations, actions_mean), dim=-1))
