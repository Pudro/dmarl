...
class MAPPO_Trainer(Base_Trainer):
    ...
    def __init__(self, agent_config: Namespace, env: MAgentEnv) -> None:
        ...
        input_dim = np.array(self.nn_agents[0].actor_net[0].in_features + np.array(self.env.state_space.shape).prod())
    ...
    def get_actions(self, observations, infos) -> dict[str, torch.Future]:
        ...
        for nn_agent in self.nn_agents:
            ...
            #     torch.tensor(observations[nn_agent.agent_name]).to(self.agent_config.device).flatten(),
        ...
        def _update_mappo():
            ...
            for step in range(self.agent_config.update_steps):
                ...
                for batch in batches:
                    ...
                    states = torch.tensor(states_arr[batch]).to(self.agent_config.device)
                    ...
                    stacked_obs = torch.cat((obs, states), dim=-1)
