# noqa
"""
## Custom Combined Arms - Ranged radius = 3
...
def generate_map(env, map_size, handles):
    ...
    for x in range(max(width // 2 - gap - side, 1), width // 2 - gap - side + side, 2):
        for y in range((height - side) // 2, (height - side) // 2 + side, 2):
            pos1[ct % 2].append([x, y])
        ct += 1

    xct1 = ct
    for x, y in pos1[0] + pos1[1]:
        if not (0 < x < width - 1 and 0 < y < height - 1):
            assert False
    env.add_agents(handles[0], method="custom", pos=pos1[0])
    env.add_agents(handles[1], method="custom", pos=pos1[1])
    ...
    for x in range(width // 2 + gap, min(width // 2 + gap + side, height - 1), 2):
        for y in range(
            ...
            pos2[ct % 2].append([x, y])
        ct += 1
        if xct1 <= ct:
            break

    for x, y in pos2[0] + pos2[1]:
        if not (0 < x < width - 1 and 0 < y < height - 1):
            assert False
    env.add_agents(handles[2], method="custom", pos=pos2[0])
    env.add_agents(handles[3], method="custom", pos=pos2[1])

    walls = []

    for h in range(height):
        if h not in (17,18,19,20):
            walls.append((width//2 + 1, h))

    walls.append((width//2 - 2, 16))
    walls.append((width//2 - 1, 16))
    walls.append((width//2, 16))
    walls.append((width//2 + 1, 16))
    walls.append((width//2 - 2, 21))
    walls.append((width//2 - 1, 21))
    walls.append((width//2, 21))
    walls.append((width//2 + 1, 21))
...
class _parallel_env(magent_parallel_env, EzPickle):
    ...
    def __init__(
        ...
        names = ["redmelee", "redranged", "greenmelee", "greenranged"]
        self.names = names
        super().__init__(
            env,
            env.get_handles(),
