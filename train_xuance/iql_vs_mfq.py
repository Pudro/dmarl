...
def parse_args():
    ...
    parser.add_argument("--method", type=str, nargs="+", default=["iql", "mfq"])
    ...
    parser.add_argument("--config", type=str, nargs="+", default=["./configs/iql_magent2.yaml",
...
class Writer():
    ...
    def add_writer(self, log_dir, name):
        self.summary_writers[name] = SummaryWriter(log_dir)

    def add_scalar(self, name, *args):
        try:
            self.summary_writers[name].add_scalar(*args)
        except:
            self.summary_writers[name].add_scalars(*args)

    def add_video(self, name, *args, **kwargs):
        self.summary_writers[name].add_video(*args, **kwargs)
...
class Runner():
        ...
        for i_method, arg in enumerate(self.args):
            ...
            if arg.logger == "tensorboard":
                ...
                if not os.path.exists(log_dir):
                    os.makedirs(log_dir)
                self.writer.add_writer(log_dir, arg.agent_name)
    ...
    def log_infos(self, info: dict, x_index: int, name=None):
        """
        info: (dict) information to be visualized
        n_steps: current step
        """
        if name is None:
            name = self.marl_names[0].lower()
        ...
        if self.use_wandb:
            ...
            for k, v in info.items():
                self.writer.add_scalar(name, k, v, x_index)

    def log_videos(self, info: dict, fps: int, x_index: int = 0, name=None):
        if name is None:
            name = self.marl_names[0].lower()
        ...
        if self.use_wandb:
            ...
            for k, v in info.items():
                self.writer.add_video(name, k, v, fps=fps, global_step=x_index)
            ...
            if not self.train_per_step:
                for h, mas_group in enumerate(self.marl_agents):
                    if mas_group.args.agent_name == "random":
                        continue
                    train_info = self.marl_agents[h].train(self.current_step)
                    self.log_infos(train_info, self.current_step, name=mas_group.args.agent_name)
                    self.log_infos(episode_info, self.current_step, name=mas_group.args.agent_name)
