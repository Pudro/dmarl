...
class Runner:
    def __init__(self, config) -> None:
        ...
        # NOTE: assuming that the config is for one run - both sides in the same file
        self.config = config
        ...
        config.handle_name = self.env.side_names

        if self.n_handles > 1 and config.agent != "RANDOM":
            config.model_dir += "{}/".format(config.handle_name)

        config.n_agents = self.env.n_agents
        config.agent_keys, config.agent_ids = self.agent_keys, self.agent_ids
        config.state_space = self.env.state_space
        config.observation_space = self.env.observation_space

        if isinstance(self.env.action_space[self.agent_keys[0]], Box):
            config.dim_act = self.env.action_space[self.agent_keys[0]].shape[0]
            config.act_shape = (config.dim_act,)
        else:
            config.dim_act = self.env.action_space[self.agent_keys[0]].n
            config.act_shape = ()

        config.action_space = self.env.action_space

        if config.env_name == "MAgent2":
            config.obs_shape = (np.prod(self.env.observation_space[self.agent_keys[0]].shape),)
